    ...
    if not judge_num:
        ...
        for p in presenters:
            ...
            if current_val in ["O", "X"]:
                label_text = "공감" if current_val == "O" else "비공감"
            ...
            with col1:
                # 중복 글자 제거 (O 하나만 깔끔하게 표시)
                if st.button(f"⭕ 공감", key=f"btn_O_{p}"):
                    ...
                    st.success(f"'{p}' -> 공감 저장 완료!")
                    st.rerun()
                    
            with col2:
                # 중복 글자 제거 (X 하나만 깔끔하게 표시)
                if st.button(f"❌ 비공감", key=f"btn_X_{p}"):
                    if current_judge not in df["심사위원"].values:
                        new_row_data = {"심사위원": current_judge, "제출상태": ""}
                        for pres in presenters:
                            new_row_data[pres] = ""
    ...
    if password == ADMIN_PASSWORD:
        ...
        if os.path.exists(DATA_FILE):
            ...
            if not df.empty:
                ...
                result_df["순위"] = result_df["공감 개수"].rank(ascending=False, method="min").astype(int)
                
                st.write("### 📊 발표자별 집계 결과", result_df.sort_values(by="공감 개수", ascending=False))
            else:
